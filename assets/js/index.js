...
(function () {
     ...
     switch (currentTheme) {
          case "light":
               root.setAttribute("color-scheme", "light");
               themeButton.innerHTML = `<i class="fa-solid fa-moon"></i>`;
               themeButton.style.backgroundColor = "#f12110";
               break;
          case "dark":
...
//loading superheros on page loads
window.onload = async function () {
     // getHeros();

     let PUBLIC_KEY = "553c822565af12afcb3f9a430e6c7ca2";
     let PRIVATE_KEY = "5adf0d2cea38d99d70b4973b5f1ee7fc141f9362";

     let ts = new Date().getTime();
     let hash = CryptoJS.MD5(ts + PRIVATE_KEY + PUBLIC_KEY).toString();

     //check localstorage having data or not.If data is present then fetch data from localStorage else get data from api
     let herosCharacters = localStorage.getItem("herosCharacters");
     if (herosCharacters != null) {
          showSearchedResults(JSON.parse(herosCharacters));
     } else {
          // API call to get the data 
          await fetch(`https://gateway.marvel.com/v1/public/characters?apikey=553c822565af12afcb3f9a430e6c7ca2&hash=${hash}&ts=${ts}`)
               .then(res => res.json()) //Converting the data into JSON format
               .then(data => addToSave(data.data.results))
     }
...
async function searchHeros(textSearched) {
     ...
     // API call to get the data 
     await fetch(`https://gateway.marvel.com/v1/public/characters?nameStartsWith=${textSearched}&apikey=553c822565af12afcb3f9a430e6c7ca2&hash=${hash}&ts=${ts}`)
          .then(res => res.json()) //Converting the data into JSON format
          .then(data => showSearchedResults(data.data.results)) //sending the searched results characters to show in HTML
}
...
function showSearchedResults(searchedHero) {
     ...
     for (const key in searchedHero) {
          // if count <= 8 then only we display it in dom other results are discarded
          if (count <= 8) {
               ...
               searchResults.innerHTML +=
                    ...
                    <div class="card card-01 height-fix">
                      <img class="card-img-top" src="${hero.thumbnail.path + '/portrait_medium.' + hero.thumbnail.extension}" alt="Image">
                      <div class="card-img-overlay">
                      <a class="character-info" href="./more-info.html"><h4 class="card-title"><strong>${hero.name}</strong></h4></a>
                               <p class="card-text"> <button class="btn add-to-fav-btn" id="${hero.id}">${favouritesCharacterIDs.has(`${hero.id}`) ? "<i class=\"fa-solid fa-heart-circle-minus\"></i> &nbsp; Remove from Favourites" : "<i class=\"fa-solid fa-heart fav-icon\"></i> &nbsp; Add to Favourites</button>"}</p>
                      </div>
...
function addToFavourites() {
     ...
     // For removing the character form favourites array
          ...
          let idOfCharacterToBeRemoveFromFavourites = this.parentElement.parentElement.parentElement.children[2].children[6].innerHTML;
...
// Function invoked when "first page is load" save to localstorage
function addToSave(heros) {

     // Setting the new herosCharacters array which now has the new character 
     localStorage.setItem("herosCharacters", JSON.stringify(heros));

     //after saving result to local storage show result on page
     showSearchedResults(heros);
...
function addInfoInLocalStorage() {
     ...
     let heroInfo = {
          name: this.parentElement.parentElement.parentElement.parentElement.children[2].children[0].innerHTML,
          description: this.parentElement.parentElement.parentElement.parentElement.children[2].children[1].innerHTML,
          comics: this.parentElement.parentElement.parentElement.parentElement.children[2].children[2].innerHTML,
          series: this.parentElement.parentElement.parentElement.parentElement.children[2].children[3].innerHTML,
          stories: this.parentElement.parentElement.parentElement.parentElement.children[2].children[4].innerHTML,
          portraitImage: this.parentElement.parentElement.parentElement.parentElement.children[2].children[5].innerHTML,
          id: this.parentElement.parentElement.parentElement.parentElement.children[2].children[6].innerHTML,
          landscapeImage: this.parentElement.parentElement.parentElement.parentElement.children[2].children[7].innerHTML,
          squareImage: this.parentElement.parentElement.parentElement.parentElement.children[2].children[8].innerHTML
     }
